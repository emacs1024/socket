...
std::unique_ptr<SocketAddress> Utility::CreateAddress(struct sockaddr *sa, socklen_t sa_len)
{
    switch (sa->sa_family)
    {
        case AF_INET:
            if (sa_len == sizeof(struct sockaddr_in))
            {
                ...
                return std::unique_ptr<SocketAddress>(new Ipv4Address(*p));
            }
            break;
...
#   ifdef IPPROTO_IPV6
        case AF_INET6:
            if (sa_len == sizeof(struct sockaddr_in6))
            {
                ...
                return std::unique_ptr<SocketAddress>(new Ipv6Address(*p));
            }
            break;
#   endif//IPPROTO_IPV6
#endif//ENABLE_IPV6
    ...
    return std::unique_ptr<SocketAddress>(nullptr);
}
...
bool
Utility::reverse(struct sockaddr *sa, socklen_t sa_len, std::string& hostname, std::string& service, int flags)
{
    hostname = "";
    service = "";
#ifdef NO_GETADDRINFO
    switch (sa->sa_family)
    {
        case AF_INET:
            ...
            {
                struct sockaddr_in *sa_in = (struct sockaddr_in *) sa;
                struct hostent *h = gethostbyaddr((const char *) &sa_in->sin_addr, sizeof(sa_in->sin_addr),
                                                  AF_INET);
                if (h)
                {
                    hostname = h->h_name;
        ...
        case AF_INET6:
            ...
            else
            {
                ...
                struct hostent *h = gethostbyaddr((const char *) &sa_in->sin6_addr, sizeof(sa_in->sin6_addr),
                                                  AF_INET6);
                if (h)
                {
                    hostname = h->h_name;
