...
int Json::Token(const char *buffer, size_t& index, std::string& ord)
{
    ...
    else if (!strncmp(&buffer[index], "null", 4))   // null value
    {
        m_type = TYPE_UNKNOWN;
    ...
    else if (!strncmp(&buffer[index], "true", 4))   // Boolean: true
    {
        m_type = TYPE_BOOLEAN;
        ord = std::string(buffer + x, 4);
        m_b_value = true;
    ...
    else if (!strncmp(&buffer[index], "false", 5))   // Boolean: false
    {
        m_type = TYPE_BOOLEAN;
...
char Json::Parse(const char *buffer, size_t& index)
{
    ...
    else if (token == -TYPE_UNKNOWN)
    {
    }
    else if (token == -TYPE_BOOLEAN)
    {
    }
    else if (token == '[')   // Array
    {
        ...
        while (true)
        {
            ...
            else if (res == ',')   // another element follows
            {
            }
            else
            {
