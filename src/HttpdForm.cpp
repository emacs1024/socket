...
HttpdForm::HttpdForm(FILE *fil)
{
    ...
    if (r_m && !strcasecmp(r_m, "post"))
    {
        ...
        if (c_t && c_l)
        {
            std::unique_ptr<IFile> p = std::unique_ptr<IFile>(new File(fil));
...
HttpdForm::HttpdForm(IFile *infil,
    ...
    raw(false),
    m_file_upload(nullptr),
    m_upload_stream(nullptr)
{
...
HttpdForm::HttpdForm(const std::string& buffer, size_t l) :
    raw(false),
    m_file_upload(nullptr),
    m_upload_stream(nullptr)
{
...
void HttpdForm::ParseFormData(IFile *infil, const std::string& content_type, size_t content_length)
{
    ...
    if (content_type.size() >= 19 && content_type.substr(0, 19) == "multipart/form-data")
    {
        ...
        if (!m_strBoundary.empty())
        {
            ...
            while (cl >= 0 && !infil->eof())
            {
                ...
                current_name = "";
                current_filename = "";
                if ((strstr(slask, m_strBoundary.c_str()) || strstr(m_strBoundary.c_str(), slask)) &&
                    strcmp(slask, m_strBoundary.c_str()))
                {
                    m_strBoundary = slask;
                    l = m_strBoundary.size();
                    delete[] tempcmp;
                ...
                if (!strcmp(slask, m_strBoundary.c_str()))
                {
                    ...
                    while (cl >= 0 && !infil->eof() && *slask)
                    {
                        ...
                        else if (!strcasecmp(h.c_str(), "Content-Disposition"))
                        {
                            Parse pa(h2, ";");
                            h = pa.getword();
                        ...
                        cl -= (int) strlen(slask);
                        while (strlen(slask) &&
                               (slask[strlen(slask) - 1] == 13 || slask[strlen(slask) - 1] == 10))
                        {
                            slask[strlen(slask) - 1] = 0;
                        }
                    }
                    ...
                    if (current_filename.empty()) // not a file
                    {
                        ...
                        cl -= (int) strlen(slask);
                        while (cl >= 0 && !infil->eof() &&
                               strncmp(slask, m_strBoundary.c_str(), m_strBoundary.size()))
                        {
                            val += slask;
                            infil->fgets(slask, TMPSIZE);
                            cl -= (int) strlen(slask);
                    ...
                    else   // current_filename.size() > 0
                    {
                        ...
                        if (m_file_upload && !m_upload_stream)
                            m_upload_stream = &m_file_upload->IFileUploadBegin(current_name, current_filename,
                                                                               content_type);
                        else
                        {
                            fil = new File;
    ...
    else if (strstr(content_type.c_str(), "x-www-form-urlencoded"))
    {
