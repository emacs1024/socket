...
void TcpSocket::InitializeContext(const std::string& context, const std::string& certfile, const std::string& keyfile, const std::string& password,
                                  const SSL_METHOD *meth_in)
{
    Lock lock(m_server_ssl_mutex);
    /* Create our context*/
    if (m_server_contexts.find(context) == m_server_contexts.end())
