...
HttpdSocket::HttpdSocket(ISocketHandler& h)
    : HTTPSocket(h), m_content_length(0), m_file(NULL), m_received(0), m_request_id(++m_request_count),
      m_cookies(NULL), m_form(NULL)
{
...
void HttpdSocket::OnHeader(const std::string& key, const std::string& value)
{
    ...
    else if (!strcasecmp(key.c_str(), "if-modified-since"))
    {
