...
SctpSocket::SctpSocket(ISocketHandler& h, int type) : StreamSocket(h), m_type(type),
...
int SctpSocket::AddAddress(SocketAddress& ad)
{
    ...
    if (GetSocket() == INVALID_SOCKET)
    {
        Handler().LogError(this, "SctpSocket", -1, "AddAddress called with invalid file descriptor",
                           LOG_LEVEL_ERROR);
        return -1;
    }
...
int SctpSocket::RemoveAddress(SocketAddress& ad)
{
    ...
    if (GetSocket() == INVALID_SOCKET)
    {
        Handler().LogError(this, "SctpSocket", -1, "RemoveAddress called with invalid file descriptor",
                           LOG_LEVEL_ERROR);
        return -1;
    }
...
int SctpSocket::AddConnection(SocketAddress& ad)
{
    ...
    if (GetSocket() == INVALID_SOCKET)
    {
        Handler().LogError(this, "SctpSocket", -1, "AddConnection called with invalid file descriptor",
                           LOG_LEVEL_ERROR);
        return -1;
    }
...
void SctpSocket::OnReceiveMessage(const char *buf, size_t sz, struct sockaddr *sa, socklen_t sa_len,
                                  struct sctp_sndrcvinfo *sinfo, int msg_flags)
{
}
